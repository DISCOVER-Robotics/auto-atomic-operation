...
_ROBOT_CONFIGS: list[dict] = [
    ...
    },
    {
        ...
        "robot_xml": _ASSETS / "robots" / "panda_robotiq.xml",
...
def _load_model(cfg: dict) -> tuple[mujoco.MjModel, mujoco.MjData]:
    ...
    robot = cfg.get("robot_xml")
    if robot is not None and not robot.exists():
        pytest.skip(f"Robot XML not found: {robot}")
    from auto_atom.utils.scene_loader import load_scene

    model = load_scene(scene, [robot] if robot is not None else [])
