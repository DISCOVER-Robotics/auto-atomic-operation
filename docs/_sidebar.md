- **Getting Started**
  - [Home](README.md)
  - [CLI Reference](cli_reference.md)

- **Task Configuration**
  - [Action Space](action_space.md)
  - [Pose Randomization](randomization.md)
  - [Execution Completion Flow](execution_completion_flow.md)
  - [sim_freq & update_freq](sim_freq_update_freq.md)

- **MuJoCo Backend**
  - [Backend Conditions](mujoco_backend_conditions.md)
  - [Custom Backend](custom-backend.md)
  - [Mesh Density & Gravity](mesh_density_gravity_issue.md)
  - [Gripper Joint Semantics](gripper_joint_semantics.md)
  - [Joint State Observation](joint_state_observation.md)
  - [Pose Observation](pose_observation.md)
  - [Tactile Prefix Binding](tactile_prefix_binding.md)

- **IK & Motion Control**
  - [IK Control](ik_control.md)
  - [Mink Iterative IK](mink_iterative_ik.md)
  - [Orientation Control Chain](orientation_control_chain.md)
  - [Stack Color Blocks IK](stack_color_blocks_ik_control.md)
  - [Arc Motion Tuning](arc_motion_tuning.md)

- **Gaussian Splatting**
  - [GS Rendering Alignment](gs_rendering_alignment.md)
  - [GS Background Offset Tuning](gs_background_offset_tuning.md)
  - [GS Rendering Performance](gs_rendering_performance.md)
  - [GS Collision Box Sizing](gs_scene_collision_box_sizing.md)

- **Tools**
  - [Data Collection](data_collection.md)
  - [Policy Evaluation](policy_evaluation.md)
  - [Tune Initial State](tune_initial_state.md)
  - [Tune Randomization Extremes](tune_randomization_extremes.md)
  - [Save Rendering](save_rendering.md)
  - [Visualize PLY](visualize_ply.md)
  - [Benchmark Suite](skills/bench.md)

- **Task Tuning**
  - [Open Door Tuning](open_door_tuning.md)

- **Assets & Infrastructure**
  - [HuggingFace LFS Migration](hf_lfs.md)
  - [HF Binary Upload Fix](huggingface-binary-upload-fix.md)
  - [LFS Assets Check](LFS_assets_check_summary_2026-04-02.md)

- **Migration Notes**
  - [XML / Mesh / GS Migration](skills/xml_mesh_gs_migration_notes.md)
  - [Docsify / Pages GIF Notes](skills/docsify_github_pages_media_notes.md)
  - [Docsify Cover Color Notes](skills/docsify_cover_color_notes.md)
