"""Analytical IK backend for the P7 arm v3 + UMI gripper v3."""
...
from auto_atom.backend.mjc.ik.third_party_ik.p7_arm_v3_analytical_ik import KDL_7DOF
from auto_atom.utils.pose import PoseState, quaternion_to_rotation_matrix


class P7V3AnalyticalIKSolver:
    """Wrap ``third_party_ik.p7_arm_v3_analytical_ik.KDL_7DOF`` behind ``IKSolver``."""
    ...
    def solve(
        ...
        if target_pose_in_base.batch_size != 1:
            raise ValueError(
                "P7V3AnalyticalIKSolver.solve expects a single-env PoseState, "
...
_P7V3_ROOT_BODY = "p7_mount"
_P7V3_FLANGE_SITE = "tool_site"
_P7V3_TCP_SITE = "eef_pose"


def build_p7_v3_umi_v3_backend(
    task: Any,
    operators: Any,
) -> Any:
